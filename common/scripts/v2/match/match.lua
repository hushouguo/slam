...
Match = {
	...
	constructor = function(self, copy)
	    self.copy = copy
	    cc.ScriptDebugLog(string.format("++++++++++ match constructor"))
	...
	destructor = function(self)
	    cc.ScriptDebugLog(string.format("---------- match destructor"))	
    	cc.ScriptDebugLog(string.format("比赛结束, 胜利方: %s", (self.side_victory == Side.ALLIES and "allies" or "enemy")))
    	
    	for _, entity in pairs(self.entities) do
    		if entity.side == Side.ALLIES then -- only member
    			entity:exit_match(self)
    	    else
    	        cc.ScriptDebugLog(string.format("destroy monster: %d from match", entity.id))
    	        entity:exit_match(self)
    	        entity:destructor()
    		end
    	end
    	...
    	cc.ScriptMatchEnd(self.copy.entity_master.id, self.side_victory == self.copy.entity_master.side)
	end,
	...
	add_member = function(self, entity)
		...
		cc.ScriptDebugLog(string.format("add member: %d, %d, %s, side: %d to match", entity.id, entity.baseid, entity.base.name.cn, entity.side))
	...
	add_monster = function(self, entity_baseid, side) 
	    assert(not self.isdone)
        local entity = Entity:new(self.copy, cc.ScriptEntityNew(entity_baseid), side, nil, Span.COPY)
        assert(self.entities[entity.id] == nil)
		self.entities[entity.id] = entity
		cc.ScriptDebugLog(string.format("add monster: %d, %d, side: %d to match", entity.id, entity.baseid, side))
		table.insert(self.round_list, entity.id) -- order by add
	end,
	...
	prepare = function(self)
		...
		cc.ScriptDebugLog(string.format("match prepare, allies size: %d, enemy size: %d", 
	...
	start = function(self)
		...
		assert(self.round_entityid ~= nil)
		cc.ScriptDebugLog(string.format("比赛开始, round entityid: %d", self.round_entityid))
	...
	-- 
		...
		if not self.isdone then
        	...
        	cc.ScriptDebugLog(string.format("match done, victory: %s", (self.side_victory == Side.ALLIES and "allies" or "enemy")))
        else
            cc.ScriptErrorLog(string.format(">>>>>>> match already is done, func: %s", Function()))
		end
	end
}
