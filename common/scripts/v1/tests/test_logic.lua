...
local function unittest()
	...
	cc.EventRewardTrigger = function(entityid, mapid, eventid, content, is_update)
	...
	cc.EventShopTrigger = function(entityid, mapid, eventid, content, is_update)
    ...
    local function match_loop()
        ...
        for itemid, item in pairs(g_copy.entity_master.pack.items) do
            ...
            if item.base.category == ItemCategory.USABLE then
                rc = lua_entry_use_item(g_copy.entity_master.id, item.id)
            end
		...
		local function looking_normal_event()
        	for _, event in pairs(g_copy.scene.events) do
        	    ...
        	    then
        	        ...
        	        if rc then
						cc.WriteLog(string.format("lock event: %d, %s, coord: (%d,%d)", event.baseid, event.base.name.cn, event.coord.x, event.coord.y))
        	            return event
        	        end
        	    end
        	end
