...
Entity = {
	...
	constructor = function(self, copy, entityid, side, master, span)
		...
		self.id = entityid
		self.baseid = cc.ScriptGetEntityBaseid(entityid)
		self.base = cc.ScriptLookupTable("Entity", self.baseid)
...
--		self.record = record.unserialize(self)
	    ...
	    cc.ScriptDebugLog(string.format("++++++++++ entity: %d,%d,%s, newbie: %s, create", self.id, self.baseid, self.base.name.cn, tostring(self.newbie)))
	...
	destructor = function(self)
	    if self.master ~= nil then
    		cc.ScriptDebugLog(string.format("entity: %d,%d,%s, destroy puppet: %d, %d, %s", 
    		    self.master.id, self.master.baseid, self.master.base.name.cn,
    		    self.id, self.baseid, self.base.name.cn))	    
	    else
	        cc.ScriptDebugLog(string.format("---------- entity: %d,%d,%s, destroy", self.id, self.baseid, self.base.name.cn))
	    end
        ...
        -- Don't destroy entity of master
        if self.id ~= self.copy.entity_master.id then
	        cc.ScriptEntityDestroy(self.id)
	    end
	end,
...
function Entity:pack_init(span)
    ...
    if self.newbie then
        ...
        pack.round_mp = self.base.round_mp
		...
		for target_baseid, placeholder in pairs(self.base.init_puppets) do
            ...
            local target = Entity:new(self.copy, cc.ScriptEntityNew(target_baseid), self.side, self, pack.span) -- use owner.side
        ...
        for buff_baseid, buff_layers in pairs(self.base.init_buffs) do
        	...
        	if newbuff ~= nil then
        	    ...
        	    newbuff = Buff:new(self, cc.ScriptBuffAdd(self.id, buff_baseid, buff_layers), buff_baseid, buff_layers)
    	...
    	-- init items
    	pack.items = {}
    	for item_baseid_str, o in pairs(t.pack.items) do
            ...
            if newitem ~= nil then -- overlap number of item
                newitem:set_number(newitem.number + o.number)
                -- ?? use_times ??
            else
                newitem = Item:new(self, item_baseid, o.number)
                assert(pack.items[newitem.id] == nil)
                pack.items[newitem.id] = newitem
                newitem.use_times = o.use_times
            end
    	end

        -- init equips
        pack.equips = {}
        for slot, equip_baseid in pairs(t.pack.equips) do
            local newequip = Item:new(self, equip_baseid, 1)
            assert(pack.equips[newequip.id] == nil)
            pack.equips[newequip.id] = newequip
        end

    	-- init puppets
    	pack.puppets = {}
    	for _, target_baseid in pairs(t.pack.puppets) do
    	    local target = Entity:new(self.copy, cc.ScriptEntityNew(target_baseid), self.side, self, pack.span)
            -- TODO: 宠物的属性可能会发生变化
    	    pack.puppets[target.id] = target
    	end
    	...
    	for placeholder, target_baseid in pairs(t.pack.placeholders) do
    	    if target_baseid == self.baseid then
    	        pack.placeholders[placeholder] = self -- master placeholder
    	    else
                -- TODO: 宠物的属性可能会发生变化
        	    local target = Entity:new(self.copy, cc.ScriptEntityNew(target_baseid), self.side, self, pack.span)
        	    pack.placeholders[placeholder] = target
    	    ...
    	    -- check master is in placeholders
    	    local function find_entity(self, pack)
    	        for _, target in pairs(pack.placeholders) do 
    	            if target.id == self.id then return true end
    	        end
    	        return false
    	    end
    	    assert(find_entity(self, pack))
...
function Entity:enter_copy(copy)
    ...
    cc.ScriptDebugLog(string.format("entity: %d enter copy:%d,%s, newbie: %s", self.id, copy.baseid, copy.base.name.cn, tostring(self.newbie)))
    self.pack:notify_client()
end

function Entity:exit_copy(copy)
    assert(self.copy.id == copy.id)
    cc.ScriptDebugLog(string.format("entity: %d exit copy:%d,%s, newbie: %s", self.id, copy.baseid, copy.base.name.cn, tostring(self.newbie)))
end

function Entity:enter_scene(scene)
    cc.ScriptDebugLog(string.format("entity: %d enter scene:%d,%d,%s", self.id, scene.id, scene.baseid, scene.base.name.cn))
	-- events_accomplish
	if self.record ~= nil then
		local t = self.record[tostring(self.copy.baseid)]
		if t[tostring(self.copy_layer)] ~= nil then
			scene:do_accomplish_events(t[tostring(self.copy_layer)].events_accomplish) -- accomplish event
		end
	end
end

function Entity:exit_scene(scene)
    cc.ScriptDebugLog(string.format("entity: %d exit scene:%d,%d,%s", self.id, scene.id, scene.baseid, scene.base.name.cn))
end
...
function Entity:enter_match(match)
    ...
    if self.pack.hp == 0 then 
        ...
        -- cleanup buffs
        for buffid, _ in pairs(self.pack.buffs) do cc.ScriptBuffRemove(self.id, buffid) end
    ...
    cc.ScriptDebugLog(string.format("entity: %d enter match", self.id))
...
function Entity:exit_match(match)
	...
	-- cleanup stack_XXXX
    for cardid, _ in pairs(self.pack.stack_deal) do cc.ScriptMatchStackDealRemove(self.id, cardid) end
    for cardid, _ in pairs(self.pack.stack_hold) do cc.ScriptMatchStackHoldRemove(self.id, cardid) end
    for cardid, _ in pairs(self.pack.stack_discard) do cc.ScriptMatchStackDiscardRemove(self.id, cardid) end
    for cardid, _ in pairs(self.pack.stack_exhaust) do cc.ScriptMatchStackExhaustRemove(self.id, cardid) end
...
--    for buffid, _ in pairs(self.pack.buffs) do cc.ScriptBuffRemove(self.id, buffid) end
    ...
    cc.ScriptDebugLog(string.format("entity: %d exit match", self.id))
...
function Entity:to_die()
    if self.death then 
        cc.ScriptDebugLog(string.format("entity: %d already is death, func: %s", self.id, Function()))
        return 
    end -- perhaps take multiple damage in one round
    self.death = true
    cc.ScriptDebugLog(string.format("entity: %d,%d die", self.id, self.baseid))
    cc.ScriptMatchEventDie(self.id)
	if self.copy.scene.match ~= nil then
...
function Entity:hp_add(value)
    ...
    cc.ScriptSetCurHP(self.id, self.pack.hp)
    cc.ScriptDebugLog(string.format("entity: %d, %s, add hp value: %d, hp: %d", self.id, self.base.name.cn, value, self.pack.hp))
    if value < 0 then
...
function Entity:maxhp_add(value)
    ...
    cc.ScriptSetMaxHP(self.id, self.pack.maxhp)
    cc.ScriptDebugLog(string.format("entity: %d, %s, add maxhp value: %d, maxhp: %d", self.id, self.base.name.cn, value, self.pack.maxhp))
end
...
function Entity:mp_add(value)	
	...
	cc.ScriptSetCurMP(self.id, self.pack.mp)
	cc.ScriptDebugLog(string.format("entity: %d, %s, add mp value: %d, mp: %d", self.id, self.base.name.cn, value, self.pack.mp))
end
...
function Entity:strength_add(value)	
	...
	cc.ScriptSetStrength(self.id, self.pack.strength)
	cc.ScriptDebugLog(string.format("entity: %d, %s, add strength value: %d, strength: %d", self.id, self.base.name.cn, value, self.pack.strength))
end
...
function Entity:armor_add(value)
	...
	cc.ScriptSetArmor(self.id, self.pack.armor)
	cc.ScriptDebugLog(string.format("entity: %d, %s, add armor value: %d, armor: %d", self.id, self.base.name.cn, value, self.pack.armor))
end
...
function Entity:shield_add(value)
	...
	cc.ScriptDebugLog(string.format("entity: %d, %s, add shield value: %d, shield: %d", self.id, self.base.name.cn, value, self.pack.shield))
end
...
function Entity:stack_hold_newone(card_baseid)
	...
	cc.ScriptDebugLog(string.format("entity: %d, newone card: %d,%d", self.id, card.id, card.baseid))
	return card
end
...
function Entity:buff_add(buff_baseid, buff_layers)
    local buff_base = cc.ScriptLookupTable("Buff", buff_baseid)
    assert(buff_base ~= nil)
    
    -- check span
    if buff_base.span ~= Span.NONE and buff_base.span ~= self.pack.span then
        cc.ScriptErrorLog(string.format(">>>>>>> buff_base: %d, buff_base.span: %s, entity.span: %d func: %s", buff_baseid, tostring(buff_base.span), self.pack.span, Function()))
        return
    end    
	...
	if buff ~= nil then
	    ...
	    buff = Buff:new(self, buff_baseid, buff_layers)
		assert(self.pack.buffs[buff.id] == nil)
		self.pack.buffs[buff.id] = buff
...
function Entity:add_card(card_baseid)
    ...
    cc.ScriptDebugLog(string.format("entity: %d add card: %d,%d to Entity", self.id, card.id, card.baseid))
    cc.ScriptBagAddCard(self.id, card.id)
    return card
end
...
function Entity:remove_card(cardid)
    ...
    cc.ScriptDebugLog(string.format("entity: %d remove card: %d from Entity", self.id, cardid))
    cc.ScriptBagRemoveCard(self.id, cardid)
end
...
function Entity:add_item(item_baseid, item_number)
    ...
    if item ~= nil then -- overlap number of item
        ...
        cc.ScriptBagSetItemNumber(self.id, item.id, item.number)
        item_number = item_number - item.number
    end
    ...
    while item_number > 0 and table.size(self.pack.items) < self.base.max_pack_slots do
        ...
        cc.ScriptDebugLog(string.format("entity: %d add item: %d,%d to Entity", self.id, item.id, item.baseid))
        cc.ScriptBagAddItem(self.id, item.id)
        item_number = item_number - item.number
...
function Entity:remove_item(itemid, item_number)
    ...
    if item.number > 0 then
        ...
        cc.ScriptDebugLog(string.format("entity: %d remove item: %d from Entity", self.id, itemid))
        cc.ScriptBagRemoveItem(self.id, itemid)
    end
end
...
function Entity:add_equip(itemid, slot)
    ...
    cc.ScriptBagRemoveItem(self.id, equip.id)
    self.pack.equips[slot] = equip
    cc.ScriptEquipBarAdd(self.id, slot, equip.id)
    cc.ScriptDebugLog(string.format("entity: %d add equip: %d,%d to slot: %d", self.id, equip.id, equip.baseid, slot))
    return item
end
...
function Entity:remove_equip(slot)
    ...
    if equip == nil then 
        cc.ScriptDebugLog(string.format("slot: %d is nil", slot))
        return false 
    end
    assert(self.pack.items[equip.id] == nil)
    self.pack.items[equip.id] = equip
    cc.ScriptBagAddItem(self.id, equip.id)
    self.pack.equips[slot] = nil
    cc.ScriptEquipBarRemove(self.id, slot)
    cc.ScriptDebugLog(string.format("entity: %d remove equip: %d from slot: %d", self.id, equip.id, slot))
    return true
end
...
function Entity:add_puppet(target_baseid)
    local target = Entity:new(self.copy, cc.ScriptEntityNew(target_baseid), self.side, self, self.pack.span) -- use owner.side
    assert(self.pack.puppets[target.id] == nil)
    self.pack.puppets[target.id] = target    
    cc.ScriptBagAddPuppet(self.id, target.id)
    cc.ScriptDebugLog(string.format("entity: %d add puppet: %d,%d,%s to Entity", self.id, target.id, target.baseid, target.base.name.cn))
    return target
end
...
function Entity:remove_puppet(target_entityid)    
    ...
    if target == nil then
        cc.ScriptErrorLog(string.format(">>>>>> not found puppet: %d, func: %s", target_entityid, Function()))
        return false
    end    

    -- remove puppet
    self.pack.puppets[target.id] = nil
    cc.ScriptBagRemovePuppet(self.id, target.id)
    
    target:destructor()
    cc.ScriptDebugLog(string.format("entity: %d remove puppet: %d,%d,%s from Entity", self.id, target.id, target.baseid, target.base.name.cn))
...
function Entity:arrange_placeholder(target_entityid, placeholder)
    cc.ScriptDebugLog(string.format("entity: %d, arrange placeholder, target_entityid: %d, placeholder: %d", self.id, target_entityid, placeholder))

    -- check target_entityid is valid
    if target_entityid == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> target_entityid is nil, func: %s", Function()))
        return false
    end
    ...
    if t[placeholder] == nil then
        cc.ScriptErrorLog(string.format(">>>>>> entity: %d, illegal placeholder: %d, func: %s", self.id, placeholder, Function()))
        return false
    end
    ...
    if target_entityid == 0 then
        ...
        if target.id == self.id then
            cc.ScriptErrorLog(string.format(">>>>>> entity: %d, take back master not allow, func: %s", self.id, Function()))
            return false
        end
        
        self.pack.placeholders[placeholder] = nil
        cc.ScriptBagArrangePlaceholder(self.id, 0, placeholder) -- clear placeholder
        
        assert(self.pack.puppets[target.id] == nil)
        self.pack.puppets[target.id] = target
        cc.ScriptBagAddPuppet(self.id, target.id) -- set puppets
        
        return true
    end

    if placeholder == Placeholder.NONE then
        cc.ScriptErrorLog(string.format(">>>>>> entity: %d, move to Placeholder.NONE not allow, func: %s", self.id, Function()))
        return false
    end
    ...
    if target ~= nil then
        ...
        if target_old ~= nil and target_old.id == self.id then
            cc.ScriptErrorLog(string.format(">>>>>> entity: %d, move master not allow, placeholder: %d, func: %s", self.id, placeholder, Function()))
            return false
        end

        self.pack.puppets[target_entityid] = nil
        cc.ScriptBagRemovePuppet(self.id, target.id) -- clear puppets
        ...
        if target_old ~= nil then            
            ...
            cc.ScriptBagAddPuppet(self.id, target_old.id) -- set puppets
        end

        self.pack.placeholders[placeholder] = target
        cc.ScriptBagArrangePlaceholder(self.id, target.id, placeholder) -- set placeholder
    ...
    if target == nil or target_placeholder == nil then
        cc.ScriptErrorLog(string.format(">>>>>> entity: %d, target: %s not found or placeholder: %s not found, func: %s", self.id, tostring(target_entityid), tostring(target_placeholder), Function()))
        return false
    end
    
    self.pack.placeholders[target_placeholder] = nil
    cc.ScriptBagArrangePlaceholder(self.id, 0, target_placeholder) -- clear placeholders
    
    local target_old = self.pack.placeholders[placeholder]
    if target_old ~= nil then
        self.pack.placeholders[placeholder] = nil
        cc.ScriptBagArrangePlaceholder(self.id, 0, placeholder) -- clear placeholders

        self.pack.placeholders[target_placeholder] = target_old
        cc.ScriptBagArrangePlaceholder(self.id, target_old.id, target_placeholder) -- set placeholders
    end

    self.pack.placeholders[placeholder] = target
    cc.ScriptBagArrangePlaceholder(self.id, target.id, placeholder) -- set placeholders
...
function Entity:add_gold(value)
    ...
    cc.ScriptDebugLog(string.format("entity: %d, add_gold: %d, gold: %d", self.id, value, self.pack.gold))
    cc.ScriptBagSetGold(self.id, self.pack.gold, gold_old)
end

--
-- void set_discount(value)
--
function Entity:set_discount(value)
    assert(self.pack.discount >= 0)
    local discount_old = self.pack.discount
    self.pack.discount = value;
    if self.pack.discount < 0 then self.pack.discount = 0 end
    cc.ScriptDebugLog(string.format("entity: %d, set discount from: %d to: %d", self.id, discount_old, self.pack.discount))
    -- TODO: trigger shop event if it exist
end
...
function Entity:round_begin()
    ...
    if self.death then
        cc.ScriptErrorLog(string.format(">>>>>>> entity: %d is die, func: %s", self.id, Function()))
        return
    end
    
	cc.ScriptDebugLog(string.format("*************************************** 回合: %d, entity: %d ******************************", self.copy.scene.match.round_total, self.id))
	...
	cc.ScriptDebugLog(string.format("entity: %d,%d,%s card_deal end", self.id, self.baseid, self.base.name.cn))
...
function Entity:card_deal()
	...
	cc.ScriptDebugLog(string.format("[%s:%d:%d] 发牌", self.base.name.cn, self.id, self.baseid))
	...
	while round_cards > 0 do
		...
		if table.size(self.pack.stack_deal) == 0 then
		    -- cc.ScriptDebugLog(string.format(">>>>>>> entity: %d, cards not enough for cardDeal, func: %s", self.id, Function()))
			break
		end
		...
		cc.ScriptDebugLog(string.format("    card: %s,%d,%d", card.base.name.cn, card.id, card.baseid))
	end
...
function Entity:card_play_judge(cardid, pick_entityid)
	...
	if not card.base.enable_play then
	    -- cc.ScriptDebugLog(string.format(">>>>>>> entity: %d, card_baseid: %d play card not allow", self.id, card.baseid))
		return false
	end
	...
	if card.base.require_career ~= 0 and card.base.require_career ~= self.base.career then 
	    -- cc.ScriptDebugLog(string.format(">>>>>>> entity: %d, card_baseid: %d, career dismatch", self.id, card.baseid))
		return false
	end
	...
	if self.pack.mp < card.base.cost_mp then
	    -- cc.ScriptDebugLog(string.format(">>>>>>> entity: %d, card_baseid: %d, mp not enough", self.id, card.baseid))
		return false
	end
...
--        local rc = targets ~= nil and targets[pick_entityid] ~= nil 
--        cc.ScriptDebugLog(string.format(">>>>>>> entity: %d, card_play_judge: %d,%d,%s, rc: %s, func: %s", 
--            self.id, card.id, card.baseid, card.base.name.cn, tostring(rc), Function()))
	end
...
function Entity:card_play(cardid, pick_entityid)
	...
	local card = self.pack.stack_hold[cardid]
	if card == nil then
	    cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, cardid: %d not exist, func: %s", self.id, cardid, Function()))
		return false
	end	

	if not card.base.enable_play then
	    cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, card_baseid: %d play card not allow, func: %s", self.id, card.baseid, Function()))
		return false
	end
	...
	if card.base.require_career ~= 0 and card.base.require_career ~= self.base.career then 
	    cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, card_baseid: %d, career dismatch, func: %s", self.id, card.baseid, Function()))
		return false
	end
	...
	-- cost mp
	if self.pack.mp < card.base.cost_mp then 
	    cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, card_baseid: %d, mp not enough, func: %s", self.id, card.baseid, Function()))
	    return false
	end
	...
	cc.ScriptDebugLog(string.format("[%s:%d:%d] 出牌    ---> card: [%s:%d:%d]", 
	...
	--elseif card.base.into_destroy then
    	...
    	cc.ScriptDebugLog(string.format("[%s:%d:%d] 销毁卡牌     , card: [%s:%d:%d]", 
    	    self.base.name.cn, self.id, self.baseid,
    	    card.base.name.cn, card.id, card.baseid
    	    ))
		cc.ScriptCardDestroy(self.id, cardid)
	end
...
function Entity:card_discard_judge(cardid)
	...
	if not card.base.enable_discard then
	    -- cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, card_baseid: %d, disable discard", self.id, card.baseid))
		return false
	end
...
function Entity:card_discard(cardid, passive)
	...
	local card = self.pack.stack_hold[cardid]
	if card == nil then
	    cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, cardid: %d not found, func: %s", self.id, cardid, Function()))
		return
	end		
	...
	--
	if not card.base.enable_discard then
	    cc.ScriptErrorLog(string.format(">>>>>>> entity: %d, card_baseid: %d, discard card not allow, func: %s", self.id, card.baseid, Function()))
		--
    ...
    cc.ScriptDebugLog(string.format("[%s:%d:%d] 弃牌     , card: [%s:%d:%d]", 
        self.base.name.cn, self.id, self.baseid,
        card.base.name.cn, card.id, card.baseid
        ))
...
function Entity:accomplish_event(event)
    cc.ScriptDebugLog(string.format("entity:%d accomplish event: %d, %d", self.id, event.id, event.baseid))    
    cc.ScriptTriggerEventAccomplish(self.id, event.id)
end
...
function Entity:event_reward(args, reward_index, entry_index)
    ...
    if args[reward_index] == nil then -- entity cancel reward
        cc.ScriptErrorLog(string.format(">>>>>>> args.rewards not reward_index: %s, func: %s", tostring(reward_index), Function()))
        return false
    end
    ...
    local function reward_cards(self, cards, entry_index)
        if cards[entry_index] == nil then
            cc.ScriptErrorLog(string.format(">>>>>>> reward.cards not entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
        end
        local card_baseid = cards[entry_index]
        local card_base = cc.ScriptLookupTable("Card", card_baseid)
        if card_base == nil then
            cc.ScriptErrorLog(string.format(">>>>>>> reward found illegal card_baseid: %s, func: %s", tostring(card_baseid), Function()))
            return false
        end

		if table.size(self.pack.cards) >= self.base.max_cards then
			cc.ScriptErrorLog(string.format(">>>>>>> pack.cards is full, max: %d, func: %s", self.base.max_cards, Function()))
			return false
		end

        cc.ScriptDebugLog(string.format("entity: %d, reward card: %d,%s", self.id, card_baseid, card_base.name.cn))
    ...
    local function reward_items(self, items, entry_index)
        if items[entry_index] == nil then
            cc.ScriptErrorLog(string.format(">>>>>>> rewards.items not entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
        end
		local t = items[entry_index]
		if type(t) ~= "table" then
            cc.ScriptErrorLog(string.format(">>>>>>> rewards.items not table, entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
		end
		if table.size(t) == 0 then
            cc.ScriptErrorLog(string.format(">>>>>>> rewards.items empty table, entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
		end

		for baseid, number in pairs(t) do
			local item_base = cc.ScriptLookupTable("Item", baseid)
			if item_base == nil then
				cc.ScriptErrorLog(string.format(">>>>>>> reward found illegal item_baseid: %s, func: %s", tostring(baseid), Function()))
				return false
			end

			if table.size(self.pack.items) >= self.base.max_pack_slots then
				cc.ScriptErrorLog(string.format(">>>>>>> pack.items is full, max: %d, func: %s", self.base.max_pack_slots, Function()))
				return false
			end

			cc.ScriptDebugLog(string.format("entity: %d, reward item: %d,%s, number: %d", self.id, baseid, item_base.name.cn, number))
			self:add_item(baseid, number)
		end
        return true
    end
    ...
    local function reward_puppets(self, puppets, entry_index)
        if puppets[entry_index] == nil then
            cc.ScriptErrorLog(string.format(">>>>>>> rewards.puppets not entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
        end
        local entity_baseid = puppets[entry_index]
        local entity_base = cc.ScriptLookupTable("Entity", entity_baseid)
        if entity_base == nil then
            cc.ScriptErrorLog(string.format(">>>>>>> reward found illegal entity_baseid: %s, func: %s", tostring(entity_baseid), Function()))
            return false
        end
        local size = table.size(self.pack.puppets)
		if size >= self.base.max_puppets then
            cc.ScriptErrorLog(string.format(">>>>>>> pack.puppets is full, max: %d, func: %s", self.base.max_puppets, Function()))
            return false
        end
        cc.ScriptDebugLog(string.format("entity: %d, reward puppet: %d,%s", self.id, entity_baseid, entity_base.name.cn))
    ...
    elseif reward.gold ~= nil then
        cc.ScriptDebugLog(string.format("entity: %d, reward gold: %d", self.id, reward.gold))
        self:add_gold(reward.gold)
    elseif reward.puppets ~= nil then
        return reward_puppets(self, reward.puppets, entry_index)
    else
        cc.ScriptErrorLog(string.format(">>>>>>> unhandle reward, func: %s", Function()))
...
function Entity:purchase_card(card_baseid, args)
    ...
    if args.cards == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> args.cards is nil, func: %s", Function()))
        return false
    end
    ...
    if not check_card_baseid(args, card_baseid) then
        cc.ScriptErrorLog(string.format(">>>>>>> not found card_baseid: %d in args, func: %s", card_baseid, Function()))
        return false
    end

    local card_base = cc.ScriptLookupTable("Card", card_baseid)
    if card_base == nil or card_base.price_gold == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> illegal card_baseid: %d, func: %s", card_baseid, Function()))
        return false
    end
    ...
    if self.pack.gold < price_gold then
        cc.ScriptErrorLog(string.format(">>>>>>> not enough gold: %d, price: %d, func: %s", self.pack.gold, price_gold, Function()))
        return false
    end
    ...
    self:add_card(card_baseid)    
    cc.ScriptDebugLog(string.format("entity: %d, buy card: %d,%s, cost gold: %d", self.id, 
...
function Entity:purchase_item(item_baseid, args)
    ...
    if args.items == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> args.items is nil, func: %s", Function()))
        return false
    end
    ...
    if not check_item_baseid(args, item_baseid) then
        cc.ScriptErrorLog(string.format(">>>>>>> not found item_baseid: %d in args, func: %s", item_baseid, Function()))
        return false
    end

    local item_base = cc.ScriptLookupTable("Item", item_baseid)
    if item_base == nil or item_base.price_gold == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> illegal item_baseid: %d, func: %s", item_baseid, Function()))
        return false
    end
    ...
    cc.ScriptDebugLog(string.format("entity: %d, buy item: %d,%s, cost gold: %d", self.id, 
...
function Entity:destroy_card(cardid, args)
    ...
    if args.price_gold == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> args.price_gold is nil, func: %s", Function()))
        return false
    end

    local card = self.pack.cards[cardid]
    if card == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> not found cardid: %d, func: %s", cardid, Function()))
        return false
    end
    ...
    cc.ScriptDebugLog(string.format("entity: %d, destroy card: %d,%d,%s, cost gold: %d", self.id, 
...
function Entity:levelup_card(cardid, args)
	...
	if card_baseid_new == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> card_old.levelup_baseid is nil: %d, func: %s", card_old.baseid, Function()))
        return false 
	end

    if card_baseid_new == 0 then
        cc.ScriptErrorLog(string.format(">>>>>>> card already is top level, func: %s", Function()))
        return false 
    end
    
    if cc.ScriptLookupTable("Card", card_baseid_new) == nil then 
        cc.ScriptErrorLog(string.format(">>>>>>> not exist levelup_baseid: %d card, func: %s", card_baseid_new, Function()))
        return false 
    end
    ...
    cc.ScriptDebugLog(string.format("entity: %d, levelup card: %d,%d,%s to card: %d,%d,%s, cost gold: %d", self.id, 
...
function Entity:levelup_puppet(target_entityid, args)
    ...
    if target_old == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> target_entityid: %d not found, func: %s", target_entityid, Function()))
        return false
    end

    local target_baseid_new = target_old.base.levelup_baseid
	if target_baseid_new == nil then
        cc.ScriptErrorLog(string.format(">>>>>>> target.levelup_baseid is nil: %d, func: %s", target_old.baseid, Function()))
        return false 
	end

    if target_baseid_new == 0 then
        cc.ScriptErrorLog(string.format(">>>>>>> puppet already is top level, func: %s", Function()))
        return false 
    end
    
    if cc.ScriptLookupTable("Entity", target_baseid_new) == nil then 
        cc.ScriptErrorLog(string.format(">>>>>>> not exist levelup_baseid: %d entity, func: %s", target_baseid_new, Function()))
        return false 
    end
    ...
    cc.ScriptDebugLog(string.format("entity: %d, levelup puppet: %d,%d,%s to entity: %d,%d,%s, placeholder: %d, cost gold: %d", self.id, 
...
function Entity:use_item(itemid)
    ...
    if item.base.span ~= Span.NONE and item.base.span ~= self.pack.span then
        cc.ScriptErrorLog(string.format(">>>>>>> item.baseid: %d, item.span: %d, entity.span: %d func: %s", item.baseid, item.base.span, self.pack.span, Function()))
        return false
    end

    -- check use times
    if item.base.consumable_times ~= 0 and item.use_times >= item.base.consumable_times then
        cc.ScriptErrorLog(string.format(">>>>>>> item.baseid: %d, item.consumable_times: %d, item.use_times: %d func: %s", item.baseid, item.base.consumable_times, item.use_times, Function()))
        return false
    end
    ...
    if item.base.category == ItemCategory.USABLE then
        ...
        if item.number < 1 then
            cc.ScriptErrorLog(string.format(">>>>>>> item.baseid: %d, item.number: %s, func: %s", item.baseid, tostring(item.number), Function()))
            return false
        end
    ...
    elseif item.base.category == ItemCategory.EQUIPABLE then
		...
		if table.size(self.pack.equips) >= self.base.max_equip_slots then
			cc.ScriptErrorLog(string.format(">>>>>>> pack.equips is full, max: %d func: %s", self.base.max_equip_slots, Function()))
			return false
		end
    ...
    elseif item.base.category == ItemCategory.MATERIAL then
        ...
        cc.ScriptErrorLog(string.format(">>>>>>> item_baseid: %d, category: %s, func: %s", item.baseid, tostring(item.base.category), Function()))
        return false
    end
