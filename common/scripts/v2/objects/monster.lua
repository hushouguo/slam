...
function Monster:update(delta)
	...
	if match == nil then 
--		cc.ScriptDebugLog(string.format(">>>>>>>>>> monster found match is nil"))
		return end -- match is over
	if match.isdone then 
--		cc.ScriptDebugLog(string.format(">>>>>>>>>> monster found match is done"))
		return end -- match is done
	if match.round_entityid ~= self.entity.id then 
--		cc.ScriptDebugLog(string.format(">>>>>>>>>> monster.id: %d, round_entityid: %d", self.entity.id, match.round_entityid))
