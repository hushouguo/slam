...
bool luaT_message_parser_encode(MessageParser* parser, lua_State* L, msgid_t msgid, void* buf, size_t& bufsize) {
	...
	if (byteSize > bufsize) {
		Alarm("bufsize: %ld(need: %ld) overflow for message: %s", bufsize, byteSize, message->GetTypeName().c_str());
		bufsize = byteSize;	// notify resize buf
		return false;
	}
