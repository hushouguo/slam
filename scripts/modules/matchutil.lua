...
local SPLBUFF = require("modules.splecialbuff")
...
M.calcCardAttack = function(player, card_attack)
    ...
    -- buff 影响
    local str_amount = EntityExt.getBuffLayers(player, SPLBUFF.STR) -- 力量
    ...
    math.floor((card_attack + str_amount + str_attr) * math.clamp01(1 - weak_value)))
    ...
    local cardSettleDamage = function(player, target_ids, card_base)
        ...
        for _ = 1, count do
            for _, id in pairs(target_ids) do
                ...
                EntityExt.takeDamage(target, attack, damage_type)

                -- 荆棘BUFF
                local thron_buff, thorn_value = EntityExt.getBuff(target, SPLBUFF.THORN)
