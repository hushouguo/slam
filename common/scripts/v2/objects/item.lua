...
Item = {
	...
	constructor = function(self, entity, item_baseid, item_number)
    	local base = cc.ScriptLookupTable("Item", item_baseid)
	    ...
	    self.entity = entity
		self.id = cc.ScriptItemNew(self.entity.id, item_baseid, item_number)
		...
		cc.ScriptDebugLog(string.format("entity: %d,%s, create item: %d, %d, %s, number: %d", 
		    self.entity.id, self.entity.base.name.cn, self.id, self.baseid, self.base.name.cn, self.number))
	end,
	
	destructor = function(self)
		cc.ScriptDebugLog(string.format("entity: %d,%s, destroy item: %d, %d, %s, number: %d", 
		    self.entity.id, self.entity.base.name.cn, self.id, self.baseid, self.base.name.cn, self.number))

        cc.ScriptItemDestroy(self.entity.id, self.id)
	end
}

function Item:set_number(item_number)
    cc.ScriptDebugLog(string.format("entity: %d,%s, set item:%d,%d,%s number from %d to %d",
...
function Item:init_field()
    ...
    self.base = {}
    self.base.__base = cc.ScriptLookupTable("Item", self.baseid)
...
function Item:check_field(name)
    ...
    for _, reserve_field in pairs(reserve_fields) do
        if reserve_field == name then
            cc.ScriptErrorLog(string.format(">>>>>>> Item.baseid: %d set_field, name: %s is a reserved field", self.baseid, tostring(name)))
            return false
        end
    end
    return true
...
function Item:set_field(name, value)
    ...
    if type(value) ~= type(self.base[name]) then
        cc.ScriptErrorLog(string.format(">>>>>>> Item.baseid: %d set_field, name: %s, value: %s, type(value): %s, type(self.base[name]): %s",
    ...
    cc.ScriptDebugLog(string.format("Item: %d, set_field: %s to value: %s, old: %s", self.baseid, tostring(name), tostring(value), tostring(self.base.__base[name])))
    cc.ScriptItemSetField(self.entity.id, self.id, name, tostring(self.base[name]))
end
...
function Item:reset_field(name)
    ...
    cc.ScriptDebugLog(string.format("Item: %d, reset_field: %s to value: %s", self.baseid, tostring(name), tostring(self.base[name])))
    cc.ScriptItemSetField(self.entity.id, self.id, name, tostring(self.base[name]))
end
...
function Item:reset_allfields()
    ...
    -- cc.ScriptDebugLog(string.format("Item: %d, reset_allfields", self.baseid))
    -- TODO: notify client ??
end
