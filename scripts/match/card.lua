...
function Card:init_field()
    ...
    self.base = {}
    self.base.__base = cc.LookupTable("Card", self.baseid)
...
function Card:set_field(name, value)
    ...
    if type(value) ~= type(self.base[name]) then
        cc.WriteLog(string.format(">>>>>>> Card.baseid: %d set_field, name: %s, value: %s, type(value): %s, type(self.base[name]): %s", 
...
function Card:reset_allfields()
    ...
    -- cc.WriteLog(string.format("Card: %d, reset_allfields", self.baseid))
    -- TODO: notify client ??
end
