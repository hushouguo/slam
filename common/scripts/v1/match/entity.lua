...
function Entity:buff_add(buff_baseid, buff_layers)
    local buff_base = cc.LookupTable("Buff", buff_baseid)
    assert(buff_base ~= nil)
    
    -- check span
    if buff_base.span ~= Span.NONE and buff_base.span ~= self.pack.span then
        cc.WriteLog(string.format(">>>>>>> buff_base: %d, buff_base.span: %s, entity.span: %d func: %s", buff_baseid, tostring(buff_base.span), self.pack.span, Function()))
        return
    end    
...
function Entity:remove_equip(slot)
    ...
    if equip == nil then 
        cc.WriteLog(string.format("slot: %d is nil", slot))
        return false 
    end
...
function Entity:set_discount(value)
    ...
    cc.WriteLog(string.format("entity: %d, set discount from: %d to: %d", self.id, discount_old, self.pack.discount))
...
--        cc.WriteLog(string.format(">>>>>>> entity: %d, card_play_judge: %d,%d,%s, rc: %s, func: %s", 
...
function Entity:event_reward(args, reward_index, entry_index)
    ...
    local function reward_items(self, items, entry_index)
		...
		if type(t) ~= "table" then
            cc.WriteLog(string.format(">>>>>>> rewards.items not table, entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
		end
		if table.size(t) == 0 then
            cc.WriteLog(string.format(">>>>>>> rewards.items empty table, entry_index: %s, func: %s", tostring(entry_index), Function()))
            return false
		end

		for baseid, number in pairs(t) do
			local item_base = cc.LookupTable("Item", baseid)
			if item_base == nil then
				cc.WriteLog(string.format(">>>>>>> reward found illegal item_baseid: %s, func: %s", tostring(baseid), Function()))
				return false
			end
			...
			cc.WriteLog(string.format("entity: %d, reward item: %d,%s, number: %d", self.id, baseid, item_base.name.cn, number))
