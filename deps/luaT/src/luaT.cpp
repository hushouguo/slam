/*
 * \file: luaT.cpp
 * \brief: Created by hushouguo at 09:25:09 Apr 15 2019
 */
...
namespace luaT {
		...
		bool luaT_pcall(lua_State* L, int args, luaT_Value& ret) {
			...
			if (!lua_isfunction(L, func_idx)) {
				Error("value at stack [%d] is not function", func_idx);
			...
			if (error) {
				if (traceback == 0) {
					Error("%s", lua_tostring(L, -1));/* ... error */
		...
		void luaT_tracestack(lua_State* L) {
			...
			for (int i = 1; i <= args; ++i) {
				Debug("    [%d] %s(%s)", i, luaT_tostring(L, i), lua_typename(L, lua_type(L, i)));
	...
	template <typename T> T* luaT_get_object(lua_State* L, const char* name) {
		...
		template <> void luaT_pushvalue(lua_State* L, luaT_Value value) {
			switch (value.type) {
				default: Error("unknown value.type: %d", value.type);
