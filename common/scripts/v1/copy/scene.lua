...
Scene = {
    ...
    constructor = function(self, copy, baseid, seed, entityid, events_base)
        self.copy = copy
...
function Scene:new(copy, baseid, seed, entityid, events_base)
	local object = {}
	self.__index = self
	setmetatable(object, self)
	object:constructor(copy, baseid, seed, entityid, events_base)
	return object
end
...
function Scene:dispatch_event(entityid, event, is_update)
    assert(not event.accomplish)
    cc.WriteLog(string.format("eventTrigger: %d, category: %d, is_update: %s, func: %s", event.baseid, event.base.category, tostring(is_update), Function()))
    local eventDispatch = {
    	[EventCategory.ENTRY] = function(entityid, sceneid, event, content, is_update)
            -- assert(content ~= nil)
            -- table.dump(content, "event.Entry")
    	    end,

        [EventCategory.MONSTER] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.Monster")
            cc.EventMonsterTrigger(entityid, sceneid, event.id, content, is_update)
            end,
            
        [EventCategory.REWARD] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.Reward")
            cc.EventRewardTrigger(entityid, sceneid, event.id, content, is_update)
            end,

        [EventCategory.SHOP_BUY_CARD] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.BuyCard")
            cc.EventShopTrigger(entityid, sceneid, event.id, content, is_update)
            end,

        [EventCategory.SHOP_DESTROY_CARD] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.DestroyCard")
            cc.EventDestroyCardTrigger(entityid, sceneid, event.id, content, is_update)
            end,

    	[EventCategory.SHOP_LEVELUP_CARD] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.LevelupCard")
            cc.EventLevelupCardTrigger(entityid, sceneid, event.id, content, is_update)
    		end,

        [EventCategory.SHOP_LEVELUP_PUPPET] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.LevelupPuppet")
            cc.EventLevelupPuppetTrigger(entityid, sceneid, event.id, content, is_update)
            end,
        
        [EventCategory.STORY] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.Story")
            cc.EventStoryTrigger(entityid, sceneid, event.id, content, is_update)
            end,

    	[EventCategory.STORY_OPTION] = function(entityid, sceneid, event, content, is_update)
            assert(content ~= nil)
            table.dump(content, "event.StoryOption")
            cc.EventOptionTrigger(entityid, sceneid, event.id, content, is_update)
    		end,

        -- trigger event to exit map
        [EventCategory.EXIT] = function(entityid, sceneid, event, content, is_update)
    ...
    if event.base.category == EventCategory.SHOP_BUY_CARD then -- Shop can be update data
        eventDispatch[event.base.category](entityid, self.id, event, self:take_event_content(entityid, event), is_update)
	elseif event.base.category == EventCategory.SHOP_BUY_CARD then -- Reward can be update data
        ...
        eventDispatch[event.base.category](entityid, self.id, event, self:take_event_content(entityid, event), false) 
    end
...
--		if (self.FLAG_adjust_dest_event and existEvent ~= nil) or self.FLAG_adjust_dest_obstacle then
	...
	if src_coord.x == dest_coord.x and src_coord.y == dest_coord.y then
		cc.WriteLog(string.format("src_coord is the same with dest_coord"))
		return true
	end
